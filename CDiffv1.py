...
class DiffParser():
    ...
    def diff_projects(self,project1: str, project2: str) -> dict:
        ...
        :return: 更改点所处文件名、所处行号、所处函数名，字典key的格式为文件名？行号，value为函数名
        """
        self.diff_info = dict()
            ...
            for file1 in files1:
                if file1.endswith(".c"):  # 还有.h文件没考虑
                    ...
                    if os.path.isfile(file2_path):
                        ...
                        # 防止乱序，set转list并经过sort()处理
                        ls_df_p = list(df_p)
                        ls_df_f = list(df_f)
                        ls_df_p.sort()
                        ls_df_f.sort()
                        #print(dict(zip(df_p, df_f)))
                        for i in range(len(ls_df_p)):
                            self.diff_info[ls_df_p[i]] = ls_df_f[i]
                        #会乱序
                        #for func_loc, func_name in zip(df_p, df_f):
                        #    self.diff_info[func_loc] = func_name
                    else:
            ...
            for file2 in files2:
                if file2.endswith(".c"):
                    ...
                    if not os.path.isfile(file1_path):
                        print(f"{file1_path} does not exist in the second project\n")
